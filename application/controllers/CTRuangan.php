...
class CTRuangan extends CI_Controller {
	...
	function __construct() {
		...
		## load model here 
		$this->load->model('MTruangan', 'Truangan');
		$this->load->model('MRuangan', 'Ruangan');

		$this->data = array(
            'controller'=>'ctruangan',
            'redirect'=>'peminjaman-ruangan',
            'title'=>'Peminjaman Ruangan',
            'parent'=>'peminjaman',
            'ruangan' => $this->Ruangan->getList(),
        );
	...
	public function index()	{	
		...
		$data['list'] = $this->Truangan->getAll();
		$data['column'] = $this->Truangan->getColumn();	
		$data['inavailable'] = $this->Truangan->getInAvailble();

		$data['available'] = array();

		foreach ($data['ruangan'] as $key => $row) {
			if(!in_array($row->id, $data['inavailable'])) {
				array_push($data['available'], $row);
			} 
		}
		
		$this->load->view('inc/truangan/list', $data);
	}
    ...
    public function pengembalian() {        
          ...
          $data['list'] = $this->Truangan->getPengembalian();
          
          $data['column'] = $this->Truangan->getColumn();       

          $this->load->view('inc/truangan/list_pengembalian', $data);
    }
	...
	public function insert() {
		...
		$err = $this->Truangan->insert($filename);
	...
	public function edit($id) {
		...
		$data['list_edit'] = $this->Truangan->getByID($id) ;
	...
	public function update() {
		...
		$err = $this->Truangan->update($this->input->post('id'),$filename);			
